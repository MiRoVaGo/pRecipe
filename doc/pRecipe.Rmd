...
Downloading the entire data collection or only a few data sets is quite straightforward. You just call the `download_data` function, which has two arguments *name* and *destination*.
...
To subset our data to a desired region and period of interest, we use the `subset_spacetime` function, which has three arguments *data_file*, *years*, and *bbox*.
...
To further crop our data to a desired polygon other than a rectangle, we use the `crop_data` function, which has two arguments *data_file*, and *shp_path*.
...
To make a time series out of our data, we use the `make_ts` function, which has one argument *data_file*.

...
plot_taylor <- function(dummie, ref, groups = "type"){
  text_obs <- ref$name[1]
  ref <- ref[, obs := value][, c("date", "obs")]
  precip <- merge(ref, dummie, by = "date")
  ...
  p00 <- TaylorDiagram(precip, obs = "obs", mod = "value", group = "name", 
                       type = groups, xlab = NULL, annotate = NULL,
                       normalise = TRUE, auto.text = FALSE,
                       ylab = "Standard Deviation (Normalized)", rms.col = NA,
                       cols = hue_pal()(length(unique(precip$name))),
                       key.title = "Data Set",
                       text.obs = text_obs)
  return(p00)
}
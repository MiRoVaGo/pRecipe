...
mon_to_year <- function(name, database_path = "./data/database"){
  if (!Reduce("&", is.element(name, c("20cr", "chirps", "cmap", "cmorph", "cpc", "cru_ts", "em_earth", "era20c", "era5", "ghcn", "gldas_clsm", "gldas_noah", "gldas_vic", "gpcc", "gpcp", "gpm_imerg", "mswep", "ncep_doe", "ncep_ncar", "persiann", "precl", "terraclimate", "trmm_3b43", "udel")))){
    if (is(name, "RasterBrick") | is(name, "RasterLayer") | is(name, "RasterStack")){
      ...
      nc_out <- sub(".nc.*", "", nc_in)
      nc_out <- paste0(nc_out, ".yearly.nc")
      cdo_str <- paste0("cdo --no_warnings -L -z zip_4 -setmon,1 -setday,1 -yearsum ", nc_in, " ", nc_out)
      system(cdo_str)
      return(invisible())
    } else {
      stop("Error: Data set is not a raster object nor is available. Select from 20cr, chirps, cmap, cmorph, cpc, cru_ts, em_earth, era20c, era5, ghcn, gldas_clsm, gldas_noah, gldas_vic, gpcc, gpcp, gpm_imerg, mswep, ncep_doe, ncep_ncar, persiann, precl, terraclimate, trmm_3b43, udel")
    }
  }
  ...
  if ((as.numeric(start_month) != 1) & (as.numeric(end_month) != 12)){
    ...
    nc_out <- paste(nc_out, as.numeric(start_year) + 1, as.numeric(end_year) - 1, "025.yearly.nc", sep = ".")
